...
monteCarloMethod(mcSteps, protein, temperature, x_position, y_position,amino_storage);
end

function [] = monteCarloMethod(mcSteps, protein,temperature, x_position, y_position, amino_storage)
% function [] = monteCarloMethod( mcSteps, protein,...
...
aminoEnergy = randomEnergy();
initialEnergy = structureEnergy(aminoEnergy,protein,amino_storage);
proteinEnergy(1) = initialEnergy;
kB = 1.38064852E-23;
right = [1 0]; left = [-1 0]; up = [0 1]; down = [0 -1];
directions = {right, left, up, down}; %Checks areas around amino acids
tempStructure = amino_storage;
for step = 1: mcSteps - 1
    randomAminoAcid = (round((length(tempStructure)-1)*rand(1,1)+ 1));
    %Used for if the random point picked is at the very beginning of the
    %chain
    if randomAminoAcid == 1
        
        %Used for if the random point picked is at the very end of the chain
        
    elseif randomAminoAcid == length(tempStructure)
...
animateProtein(protein,x_position,y_position);

end

function [tempStructure] = firstAminoAcid(tempStructure, directions)
nextAminoAcid = tempStructure{randomAminoAcid + 1};
%Indexes at xPosition[2] yPosition[2], which is the next amino acid in the chain
...
for i=1:length(directions)
    ...
    check = checkAminoChain(newLocation,tempStructure);
    if check
        inChain = true;
    end
    if (sqrt((nextAminoAcid(2) - newLocation(2))^2 +...
            ((nextAminoAcid(1) - newLocation(1))^2)) == 1 && ~inChain)
        
        currentEnergy = structureEnergy(aminoEnergy,protein,tempStructure);
        if(exp(-(currentEnergy - proteinEnergy(step))/(kB*temperature)) > 1)
            proteinEnergy(step+1) = currentEnergy;
            fprintf('Structure Change\n');
            x_position(randomAminoAcid) = newLocation(1);
            y_position(randomAminoAcid) = newLocation(2);
            tempStructure(randomAminoAcid) = {[newLocation(1) newLocation(2)]};
            return
        else
            
            if (exp(-(currentEnergy - proteinEnergy(step))/(kB*temperature)) > 1) > rand
                proteinEnergy(step+1) = currentEnergy;
                fprintf('Structure Change\n');
                x_position(randomAminoAcid) = newLocation(1);
                y_position(randomAminoAcid) = newLocation(2);
...
function [] = lastAminoAcid(tempStructure,directions)
randomDirection = randperm(length(directions));
previousAminoAcid = tempStructure{randomAminoAcid - 1};
for i=1:length(directions)
    inChain = false;
    ...
    if (sqrt((previousAminoAcid(2) - newLocation(2))^2 + ...
            ((previousAminoAcid(1) - newLocation(1))^2)) == 1 && ~inChain)
        ...
        if((exp(-(currentEnergy - proteinEnergy(step))/(kB*temperature)) > 1))
            ...
            if (exp(-(currentEnergy - proteinEnergy(step))/(kB*temperature)) > rand)
                proteinEnergy(step+1) = currentEnergy;
                fprintf('Structure Change\n');
                x_position(randomAminoAcid) = newLocation(1);
                y_position(randomAminoAcid) = newLocation(2);
...
for i=1:length(directions)
    for j=1:length(directions)
        ...
        check1 = checkAminoChain(newLocation1,tempStructure);
        check2 = checkAminoChain(newLocation2,tempStructure);
        if check1 || check2
            inChain = true;
        end
        ...
        if (sqrt((previousAminoAcid(2) - newLocation1(2))^2 + ...
                ...
                ((nextAminoAcid(1) - newLocation2(1))^2)) == 1 && ~inChain)
            ...
            if(exp(-(currentEnergy - proteinEnergy(step))/(kB*temperature)) > 1)
                ...
                if (exp(-(currentEnergy - proteinEnergy(step))/(kB*temperature)) > rand)
                    ...
                    x_position(randomAminoAcid) = newLocation1(1);
                    y_position(randomAminoAcid) = newLocation1(2);
                    tempStructure{randomAminoAcid} = {[newLocation1(1) newLocation1(2)]};
                    break
                else
                    proteinEnergy(step+1) = proteinEnergy(step);
                ...
                function [] = animateProtein(protein, x_position,y_position)
