%Exercise 2.3, uses the taylor series approximation to compare the values   
%exp^(jx) = cos(x) + j*sin(x) and exp(-jx) = cos(x) - jsin(x) where 
% j =sqrt(-1)
function [] = expjx(x);
euler = cos(x) + (i*sin(x));
v = x*i;
...
real = exp(i*x); 
fprintf('Approximation for exp(j*x) where x = %.1f\n', x);
