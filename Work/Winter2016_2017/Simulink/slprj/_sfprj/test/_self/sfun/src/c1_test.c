...
static void sf_gateway_c1_test(SFc1_testInstanceStruct *chartInstance);
static void mdl_start_c1_test(SFc1_testInstanceStruct *chartInstance);
...
static real_T c1_emlrt_marshallIn(SFc1_testInstanceStruct *chartInstance, const
  mxArray *c1_b_outputWave, const char_T *c1_identifier);
...
static const mxArray *c1_b_sf_marshallOut(void *chartInstanceVoid, void
  *c1_inData);
static int32_T c1_c_emlrt_marshallIn(SFc1_testInstanceStruct *chartInstance,
  const mxArray *c1_u, const emlrtMsgIdentifier *c1_parentId);
static void c1_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c1_mxArrayInData, const char_T *c1_varName, void *c1_outData);
static uint8_T c1_d_emlrt_marshallIn(SFc1_testInstanceStruct *chartInstance,
...
static void init_simulink_io_address(SFc1_testInstanceStruct *chartInstance);

/* Function Definitions */
static void initialize_c1_test(SFc1_testInstanceStruct *chartInstance)
{
  if (sf_is_first_init_cond(chartInstance->S)) {
    initSimStructsc1_test(chartInstance);
  ...
  *chartInstance)
{
  ...
  c1_hoistedGlobal = *chartInstance->c1_outputWave;
  ...
  mxArray *c1_st)
{
  ...
  *chartInstance->c1_outputWave = c1_emlrt_marshallIn(chartInstance, sf_mex_dup
    (sf_mex_getcell("outputWave", c1_u, 0)), "outputWave");
  chartInstance->c1_is_active_c1_test = c1_d_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell("is_active_c1_test", c1_u, 1)),
    "is_active_c1_test");
  sf_mex_destroy(&c1_u);
...
{
  real_T c1_hoistedGlobal;
  real_T c1_b_inputWave;
  uint32_T c1_debug_family_var_map[4];
  real_T c1_nargin = 1.0;
  real_T c1_nargout = 1.0;
  real_T c1_b_outputWave;
  _SFD_SYMBOL_SCOPE_PUSH(0U, 0U);
  _sfTime_ = sf_get_time(chartInstance->S);
  _SFD_CC_CALL(CHART_ENTER_SFUNCTION_TAG, 0U, chartInstance->c1_sfEvent);
  _SFD_DATA_RANGE_CHECK(*chartInstance->c1_inputWave, 0U, 1U, 0U,
                        chartInstance->c1_sfEvent, false);
  chartInstance->c1_sfEvent = CALL_EVENT;
  _SFD_CC_CALL(CHART_ENTER_DURING_FUNCTION_TAG, 0U, chartInstance->c1_sfEvent);
  c1_hoistedGlobal = *chartInstance->c1_inputWave;
  c1_b_inputWave = c1_hoistedGlobal;
  ...
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c1_nargout, 1U, c1_sf_marshallOut,
    c1_sf_marshallIn);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c1_b_inputWave, 2U, c1_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c1_b_outputWave, 3U, c1_sf_marshallOut,
    c1_sf_marshallIn);
  CV_EML_FCN(0, 0);
  _SFD_EML_CALL(0U, chartInstance->c1_sfEvent, 4);
  c1_b_outputWave = c1_b_inputWave;
  _SFD_EML_CALL(0U, chartInstance->c1_sfEvent, -4);
  _SFD_SYMBOL_SCOPE_POP();
  *chartInstance->c1_outputWave = c1_b_outputWave;
  _SFD_CC_CALL(EXIT_OUT_OF_FUNCTION_TAG, 0U, chartInstance->c1_sfEvent);
  _SFD_SYMBOL_SCOPE_POP();
  _SFD_CHECK_FOR_STATE_INCONSISTENCY(_testMachineNumber_,
    chartInstance->chartNumber, chartInstance->instanceNumber);
  _SFD_DATA_RANGE_CHECK(*chartInstance->c1_outputWave, 1U, 1U, 0U,
                        chartInstance->c1_sfEvent, false);
}
  ...
  mxArray *c1_b_outputWave, const char_T *c1_identifier)
{
  ...
  c1_y = c1_b_emlrt_marshallIn(chartInstance, sf_mex_dup(c1_b_outputWave),
    &c1_thisId);
  sf_mex_destroy(&c1_b_outputWave);
  return c1_y;
}
...
static void c1_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c1_mxArrayInData, const char_T *c1_varName, void *c1_outData)
{
  const mxArray *c1_b_outputWave;
  const char_T *c1_identifier;
  emlrtMsgIdentifier c1_thisId;
  real_T c1_y;
  SFc1_testInstanceStruct *chartInstance;
  chartInstance = (SFc1_testInstanceStruct *)chartInstanceVoid;
  c1_b_outputWave = sf_mex_dup(c1_mxArrayInData);
  ...
  const mxArray *c1_b_is_active_c1_test, const char_T *c1_identifier)
{
  ...
  c1_thisId.fIdentifier = c1_identifier;
  c1_thisId.fParent = NULL;
  c1_y = c1_e_emlrt_marshallIn(chartInstance, sf_mex_dup(c1_b_is_active_c1_test),
    &c1_thisId);
...
static void init_simulink_io_address(SFc1_testInstanceStruct *chartInstance)
{
  chartInstance->c1_inputWave = (real_T *)ssGetInputPortSignal_wrapper
    (chartInstance->S, 0);
  chartInstance->c1_outputWave = (real_T *)ssGetOutputPortSignal_wrapper
    (chartInstance->S, 1);
}
...
{
  ((real_T *)mxGetPr((plhs[0])))[0] = (real_T)(4294289866U);
  ((real_T *)mxGetPr((plhs[0])))[1] = (real_T)(1659258782U);
  ((real_T *)mxGetPr((plhs[0])))[2] = (real_T)(1068300933U);
  ((real_T *)mxGetPr((plhs[0])))[3] = (real_T)(4058140844U);
}

mxArray* sf_c1_test_get_post_codegen_info(void);
mxArray *sf_c1_test_get_autoinheritance_info(void)
{
  ...
  {
    mxArray *mxChecksum = mxCreateString("sWJLnghy4v1OxPG61MnjRC");
  ...
  {
    mxArray* mxPostCodegenInfo = sf_c1_test_get_post_codegen_info();
...
mxArray *sf_c1_test_jit_fallback_info(void)
{
...
mxArray *sf_c1_test_updateBuildInfo_args_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c1_test(void)
{
  ...
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"outputWave\",},{M[8],M[0],T\"is_active_c1_test\",}}"
  };
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    chartInstance = (SFc1_testInstanceStruct *) chartInfo->chartInstance;
      ...
      /* do this only if simulation is starting */
      {
        ...
        if (chartAlreadyPresent==0) {
          ...
          sf_debug_set_chart_event_thresholds(sfGlobalDebugInstanceStruct,
            ...
            0,
            0);
          _SFD_SET_DATA_PROPS(0,1,1,0,"inputWave");
          _SFD_SET_DATA_PROPS(1,2,0,1,"outputWave");
...
static void chart_debug_initialize_data_addresses(SimStruct *S)
{
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    if (ssIsFirstInitCond(S)) {
      ...
      {
        _SFD_SET_DATA_VALUE_PTR(0U, chartInstance->c1_inputWave);
        _SFD_SET_DATA_VALUE_PTR(1U, chartInstance->c1_outputWave);
      }
    }
  }
}
...
{
  return "swOEYfWTuXWNNhAcURXCJUG";
}
...
{
  sf_gateway_c1_test((SFc1_testInstanceStruct*) chartInstanceVar);
}
...
static const mxArray* sf_opaque_get_sim_state_c1_test(SimStruct* S)
{
  ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
  ChartInfoStruct * chartInfo = (ChartInfoStruct *)(crtInfo->instanceInfo);
  return get_sim_state_c1_test((SFc1_testInstanceStruct*)
...
static void sf_opaque_set_sim_state_c1_test(SimStruct* S, const mxArray *st)
{
  ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
  ChartInfoStruct * chartInfo = (ChartInfoStruct *)(crtInfo->instanceInfo);
  set_sim_state_c1_test((SFc1_testInstanceStruct*)chartInfo->chartInstance, st);
}
...
static void mdlProcessParameters_c1_test(SimStruct *S)
{
  ...
  if (sf_machine_global_initializer_called()) {
    ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
    ChartInfoStruct * chartInfo = (ChartInfoStruct *)(crtInfo->instanceInfo);
    initialize_params_c1_test((SFc1_testInstanceStruct*)
...
static void mdlSetWorkWidths_c1_test(SimStruct *S)
{
  ...
  ssSetChecksum0(S,(1792919427U));
  ssSetChecksum1(S,(3281513767U));
  ssSetChecksum2(S,(1572086090U));
  ssSetChecksum3(S,(1212409493U));
