% Simulation of MUSIC, ESPRIT, MVDR, Min-Norm and Classical DOA algorithms for a uniform linear array.

doas = [-30 -5 40]*pi/180; %DOA's of signals in rad.
P = [1 1 1]; %Power of incoming signals
N = 1000; %Number of array elements
K = 40000; %Number of data snapshots
...
A = exp(-1i*2*pi*d*(0:N-1)'*sin(doas(:)'));%Nxr
% Signal and noise generation
sig = round(rand(r,K))*2-1; %rxK (Generate random BPSK symbols for each of the r signals)
